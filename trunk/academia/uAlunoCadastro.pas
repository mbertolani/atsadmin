...
type
  TfAlunoCadastro = class(TfPai)
    ...
    cdsFreqDATAFREQUENCIA: TDateField;
    cdsFreqNOMETURMA: TStringField;
    cdsFreqHORARIOENTRADA: TTimeField;
    BitBtn21: TBitBtn;
...
uses UDm, uRegiaoCadastro, uProcurar, sCtrlResize, uUtils, uListaAlunos,
  uCrTituloInclui;

{$R *.dfm}

procedure TfAlunoCadastro.DBRadioGroup1Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.FormCreate(Sender: TObject);
begin
  sCtrlResize.CtrlResize(TForm(fAlunoCadastro));
  //inherited;
...
procedure TfAlunoCadastro.BitBtn3Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.BitBtn2Click(Sender: TObject);
begin
  inherited;
...
{   if ((varform = 'consulta') or (varform = 'consultaescola') or (varform = 'consultapedagogico'))  then
   begin
     ...
     fListaAlunos.BitBtn8.Enabled := False;
     fListaAlunos.ShowModal;
     try
       PageControl1.ActivePage := TabSheet1;
       cds_cli.Close;
       cds_cli.Params[0].Clear;
       cds_cli.Params[0].AsInteger := fListaAlunos.cdsCODCLIENTE.AsInteger;
       cds_cli.Open;
    ...
    if dm.cds_repcli.Active then
     dm.cds_repcli.Close;
    dm.cds_repcli.Params[0].AsInteger := cds_CliCODCLIENTE.AsInteger;
    dm.cds_repcli.Open;
    if (Not cds_faixa.Active) then
       cds_faixa.Open;
...
procedure TfAlunoCadastro.DtSrcStateChange(Sender: TObject);
begin
  Incluir  := 'S';
  Excluir  := 'S';
  Alterar  := 'S';
...
procedure TfAlunoCadastro.cds_cliNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TfAlunoCadastro.DBEdit2Exit(Sender: TObject);
begin
  inherited;
...
begin
  ...
  begin
    codcli := cds_cliCODCLIENTE.AsInteger;
    nomecli := cds_cliNOMECLIENTE.AsString;
    fListaAlunos.edCodigo.Text := IntToStr(codcli);
    fListaAlunos.BitBtn2.Click;
  end;
  if (varform = 'consulta') then
  begin
    fListaAlunos.edNome.SetFocus;
  end;
...
procedure TfAlunoCadastro.dbeCNPJExit(Sender: TObject);
 Var str_cpf: String;
begin
...
function TfAlunoCadastro.cgc(num: string): boolean;
var
...
function TfAlunoCadastro.cpf(num: string): boolean;
var
...
procedure TfAlunoCadastro.DtSrcEStateChange(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.btnExcluirClick(Sender: TObject);
begin
 if ((PageControl1.ActivePage = TabSheet1) or (PageControl1.ActivePage = TabInternet)) then
 begin
    inherited;
...
procedure TfAlunoCadastro.cdsEnderecoCliNewRecord(DataSet: TDataSet);
begin
  inherited;
  cdsEnderecoCliCODCLIENTE.AsInteger := cds_cliCODCLIENTE.AsInteger;
...
procedure TfAlunoCadastro.DBNavigator1Click(Sender: TObject;
  Button: TNavigateBtn);
var
  BtnName: string;
  begin
...
procedure TfAlunoCadastro.SpeedButton4Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.SpeedButton5Click(Sender: TObject);
begin
  inherited;
  ...
  begin
     ...
     if ((cdsEnderecoCliTIPOEND.AsInteger = 0) and (proc_endCOUNT.AsInteger = 1)) then
     begin
...
procedure TfAlunoCadastro.PageControl1Change(Sender: TObject);
begin
  inherited;
  if ((PageControl1.ActivePage = TabSheet1) or (PageControl1.ActivePage = TabInternet))then
     MMJPanel1.Align := alBottom
...
procedure TfAlunoCadastro.btnIncluir_repClick(Sender: TObject);
begin
...
procedure TfAlunoCadastro.btnGravar_repClick(Sender: TObject);
begin
...
procedure TfAlunoCadastro.btnexcluir_repClick(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.btnCancelarClick(Sender: TObject);
begin
 if ((PageControl1.ActivePage = TabSheet1) or (PageControl1.ActivePage = TabInternet)) then
 begin
...
procedure TfAlunoCadastro.DtSrcReprStateChange(Sender: TObject);
var icomp: integer;
begin
...
procedure TfAlunoCadastro.btncancelar_repClick(Sender: TObject);
begin
 if Cancelar = 'N' then Exit;
...
procedure TfAlunoCadastro.ComboBox1Change(Sender: TObject);
var faixa : integer;
begin
  inherited;
...
procedure TfAlunoCadastro.Ficha1Click(Sender: TObject);
begin
  inherited;
  if (cds_cli.State in [dsBrowse]) then
  begin
...
procedure TfAlunoCadastro.SpeedButton6Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.DBRadioGroup2Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.TabInternetShow(Sender: TObject);
begin
  inherited;
  btnProcurar.Visible := false;
end;

procedure TfAlunoCadastro.TabSheet1Show(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.DBEdit2KeyPress(Sender: TObject;
  var Key: Char);
begin
  //inherited;
  if (key = #13) then
...
procedure TfAlunoCadastro.BitBtn5Click(Sender: TObject);
begin
//  inherited;
...
procedure TfAlunoCadastro.BitBtn6Click(Sender: TObject);
begin
  //inherited;
  if(DtSrc.DataSet.State in [dsBrowse]) then
    DtSrc.DataSet.Edit;
...
procedure TfAlunoCadastro.BitBtn7Click(Sender: TObject);
begin
//  inherited;
  if(DtSrc.DataSet.State in [dsBrowse]) then
    DtSrc.DataSet.Edit;
...
procedure TfAlunoCadastro.BitBtn8Click(Sender: TObject);
begin
  inherited;
  fRegiaoCadastro := TfRegiaoCadastro.Create(Application);
...
procedure TfAlunoCadastro.SpeedButton7Click(Sender: TObject);
var t : TUtils;
    v : double;
begin
...
procedure TfAlunoCadastro.JvDBComboBox1Exit(Sender: TObject);
{var desc: double;
    codConvenio: integer;}
begin
  inherited;
...
procedure TfAlunoCadastro.DBRadioGroup4Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.CalculaDesconto;
...
procedure TfAlunoCadastro.BitBtn10Click(Sender: TObject);
begin
  inherited;
  if(DtSrc.DataSet.State in [dsBrowse]) then
    DtSrc.DataSet.Edit;
...
procedure TfAlunoCadastro.BitBtn17Click(Sender: TObject);
begin
  inherited;
  cdsParente.Delete;
  cdsParente.ApplyUpdates(0);
...
procedure TfAlunoCadastro.SpeedButton9Click(Sender: TObject);
var t: TUtils;
begin
  if (cdsTurma.Active) then
    cdsTurma.Close;
...
procedure TfAlunoCadastro.BitBtn18Click(Sender: TObject);
begin
  inherited;
...
function TfAlunoCadastro.FormExiste(aberto: Tform): Boolean;
var
  I:Integer;
begin
    FormExiste := false;
...
procedure TfAlunoCadastro.BitBtn19Click(Sender: TObject);
...
procedure TfAlunoCadastro.Button1Click(Sender: TObject);
begin
  inherited;
...
procedure TfAlunoCadastro.DBEdit13Exit(Sender: TObject);
begin
  inherited;
...
function TfAlunoCadastro.Verifica_Campos_Em_Branco: Boolean;
var cont : integer;
begin
...
function TfAlunoCadastro.Verifica_Campos_Em_BrancoEndereco: Boolean;
var cont : integer;
begin
  Result:=True; //assume que estão todos preenchidos
...
procedure TfAlunoCadastro.cds_cliReconcileError(
...
procedure TfAlunoCadastro.cdsEnderecoCliReconcileError(
  DataSet: TCustomClientDataSet; E: EReconcileError;
  UpdateKind: TUpdateKind; var Action: TReconcileAction);
begin
  inherited;
...
procedure TfAlunoCadastro.BitBtn20Click(Sender: TObject);
var t: TUtils;
begin
...
procedure TfAlunoCadastro.btnSairClick(Sender: TObject);
begin
  inherited;
  fListaAlunos.Close;
end;

procedure TfAlunoCadastro.BitBtn21Click(Sender: TObject);
begin
  inherited;
  VCLReport1.FileName := str_relatorio + 'rel_Frequencia.rep';
  VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
  ...
  VCLReport1.Report.Params.ParamByName('PCLIENTE').Value := cds_cliCODCLIENTE.AsInteger;
  vclReport1.Execute;
end;
