...
declare variable BCII_UNIT  double precision;
declare variable II_UNIT  double precision;
...
declare variable BCII_TOTAL double precision;
declare variable II_TOTAL double precision;
...
BEGIN
   ...
   begin
     ...
     II_UNIT = new.II / new.VALOR_PRODUTO;
     BCII_UNIT = new.BCII / new.VALOR_PRODUTO;
   end
   ...
   do begin
	...
	begin
	 ...
	 BCII = UDF_ROUNDDEC((:BCII_UNIT * :valtot), 2), II = UDF_ROUNDDEC((:II_UNIT * :valtot), 2) where CODDETALHE = :coddet;
	 if ( det = det1) then
     ...
     begin
        update MOVIMENTODETALHE set FRETE = UDF_ROUNDDEC((:Frete_UNIT * :valtot), 2), valor_desconto = UDF_ROUNDDEC((:DESCONTO_UNIT * :valtot), 2), 
        valor_outros = UDF_ROUNDDEC((:OUTRAS_UNIT * :valtot), 2), valor_SEGURO = UDF_ROUNDDEC((:SEGURO_UNIT * :valtot), 2), 
        BCII = UDF_ROUNDDEC((:BCII_UNIT * :valtot), 2), II = UDF_ROUNDDEC((:II_UNIT * :valtot), 2)
        where CODDETALHE = :coddet;
        if ( det = det1) then
	      update MOVIMENTODETALHE set FRETE = :Frete_TOTAL, valor_desconto = :DESCONTO_TOTAL, valor_seguro = :SEGURO_TOTAL, 
	      valor_outros = :OUTRAS_TOTAL, BCII = :BCII_TOTAL, II = :II_TOTAL
	      where CODDETALHE = :coddet;
    end
    det1 = det1 +1;
